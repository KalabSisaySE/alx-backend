        ...
        if key and item:
            ...
            if not self.cache_data.get(key):
                ...
                if len(self.cache_data) >= BaseCaching.MAX_ITEMS:
                    most_recent = MRUCache.recent_access[-1]
                    print("DISCARD: {}".format(most_recent))
                    del self.cache_data[most_recent]
                    MRUCache.recent_access.remove(most_recent)

            self.cache_data[key] = item
            # if item already exists remove it first
            # to avoid duplicates
            if key in MRUCache.recent_access:
                MRUCache.recent_access.remove(key)
            MRUCache.recent_access.append(key)
